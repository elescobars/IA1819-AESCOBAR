 ...
 *         This class contains all search algorithms plus some utilty methods
 *         needed in those algorithm
 */
public class SearchTree {
    ...
    // we start with the initial state. check if it is goal or not and expand its
    // children if it is not the goal.
    // pop the first element of the queue and check if it is goal node. if not add
    ...
    public void depthFirstSearch() {
        ...
        while (!currentNode.getState().equals(goalSate)) {
            ...
            // we add the queue that contains the successors of the visted node to the
            // beginning of the main queue
            mainQueue.addQueue(successorsQueue);
            // successors queue should be cleared in order to store the next expaneded's
            // successors
    ...
    /**
     * Find the goal using Uniform cost algorithm. in each step the node with
     * minimum cost will be expanded
     */
    // ***************************************************************************************************
    public void uniformCostSearch() {
        ...
        Node node = new Node(root.getState());
        node.setCost(0);
        // the comparator compare the cost values and make the priority queue to be
        // sorted based on cost values
        NodePriorityComparator nodePriorityComparator = new NodePriorityComparator();
        // a queue that contains nodes and their cost values sorted. 10 is the initial
        // size
            ...
            for (String n : nodeSuccessors) {
                ...
                child.setTotalCost(
                        currentNode.getTotalCost() + Character
                                .getNumericValue(child.getState().charAt(child.getParent().getState().indexOf('0'))),
                        0);
    ...
    public void bestFirstSearch() {
        ...
        // Here we try to navigate from the goal node to its parent( and its parent's
        // parent and so on) to find the path
        NodeUtil.printSolution(currentNode, stateSets, root, time);

    }

    /**
     * Find the goal using A* algorithm. The huristic is the real cost to the
     * current node from the initial Node
    ...
    public void aStar(Heuristic heuristic) {
        ...
        while (!currentNode.getState().equals(goalSate)) {
            ...
            for (String n : nodeSuccessors) {
                ...
                nodePriorityQueue.add(child);
                if (heuristic == Heuristic.H_FOUR)
                    child.setTotalCost(
                            currentNode.getTotalCost() + Character.getNumericValue(
                                    child.getState().charAt(child.getParent().getState().indexOf('0'))),
                            heuristicFour(child.getState(), goalSate));
                if (heuristic == Heuristic.H_FIVE)
                    child.setTotalCost(
                            currentNode.getTotalCost() + Character.getNumericValue(
                                    child.getState().charAt(child.getParent().getState().indexOf('0'))),
                            heuristicFive(child.getState(), goalSate));
                if (heuristic == Heuristic.H_SIX)
                    child.setTotalCost(
                            currentNode.getTotalCost() + Character.getNumericValue(
                                    child.getState().charAt(child.getParent().getState().indexOf('0'))),
                            heuristicSix(child.getState(), goalSate));
     ...
     * the depth limit starts from 1 and it increases to the defiend value by user
    ...
    // ****************************************************************************************************
    // This heuristic estimates the cost to the goal from current state by counting
    ...
    // *************************************************************************************************
    // This heuristic estimates the cost to the goal from current state by
    // calculating the Manhathan distance from each misplaced
    ...
    // ***************************************************************************************************
    // This heurisic is almost similar to heuristicTwo except that heuristicThree
    // has higher value. when we estimate the cost
    // by calculating the manhattan distance , we underestimate this cost because we
    // do not count to the number moves the intermediate tiles shoud move
    // in order to make space for the source tile to move. therefore, we think if
    // the manhattan distance is, let say, 4, it means
    ...
    // Este metodo de heuristica agrega mas valor de diferencia cuando las
    // ubicaciones diferentes a la meta son las posiciones que no
    // son las orillas del recuadro.
    private int heuristicFour(String currentState, String goalSate) {
        int difference = 0;
        for (int i = 0; i < currentState.length(); i += 1) {
            if (currentState.charAt(i) != goalSate.charAt(i) && (i == 3 || i == 6 || i == 7 || i == 8 || i == 9))
                difference += 1;
            if (currentState.charAt(i) != goalSate.charAt(i) && (i == 2 || i == 5 || i == 4))
                difference += 2;
            if (currentState.charAt(i) != goalSate.charAt(i) && (i == 1))
                difference += 3;
        }
        return difference;
    }
    ...
    // Este metodo de heuristica agrega mas valor de diferencia cuando las
    // ubicaciones diferentes a la meta NO son las orillas del recuadro
    private int heuristicFive(String currentState, String goalSate) {
        int difference = 0;
        for (int i = 0; i < currentState.length(); i += 1) {
            if (currentState.charAt(i) != goalSate.charAt(i) && (i == 1 || i == 3 || i == 7 || i == 9))
                difference += 1;
            if (currentState.charAt(i) != goalSate.charAt(i) && (i == 2 || i == 4 || i == 5 || i == 6 || i == 8))
                difference += 2;
        }
        return difference;
    }
    ...
    // Este metodo de heuristica agrega mas valor de diferencia cuando las
    // ubicaciones diferentes a la meta NO estan en las dos diagonales que pueden
    // formarse en el recuadro
    private int heuristicSix(String currentState, String goalSate) {
        int difference = 0;
        for (int i = 0; i < currentState.length(); i += 1) {
            if (currentState.charAt(i) != goalSate.charAt(i) && (i == 1 || i == 3 || i == 5 || i == 7 || i == 9))
                difference += 1;
            if (currentState.charAt(i) != goalSate.charAt(i) && (i == 2 || i == 4 || i == 6 || i == 8))
                difference += 2;
        }
        return difference;
    }
